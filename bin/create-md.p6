...
my $usage = "Usage: $*PROGRAM -m <file> | -b <bin dir> | -h[elp] [-d <odir>, -N, -M <max>, -D]";
sub usage() {
   print qq:to/END/;
   $usage

   Reads the input module (or program files in the bin dir) and extracts properly formatted
   ...
   In program files, the comments are folded into lines no longer than the
   maximum line length.  If the program has a help option (-h), the result
   of that command will be added to the output as a code block.

   See the example directory for a module file and a program with the
   known formats.  The markdown files in the docs directory
   in this repository were created with this program from those example
   file.
